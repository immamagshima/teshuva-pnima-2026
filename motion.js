...
(()=>{
 ...
 const words=p.voices||[];const active=!!words.length&&!p.listeningArt&&!state.ground&&(!p.linePortrait||e>=p.voiceAfter||!!window.thoughtsLive);
...
(()=>{
 ...
 const t=paused?hold:elapsed()+bias;const pace=Math.max(.25,Math.min(1,state.total/((current().letterForms?.length||1)*32.8))),lengths=[24,36,28,44,32].map(x=>x*pace),sum=164*pace;let local=((t%sum)+sum)%sum,k=0;while(local>=lengths[k])local-=lengths[k++];const u=local/lengths[k];
 ...
 const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches||document.body.classList.contains('wave-paused');
 const nodes=[...title.children],forms=current().letterForms||[word],ordinal=Math.floor(t/sum)*5+k,from=ordinal===0?word:forms[(ordinal-1)%forms.length],to=forms[ordinal%forms.length];const norm=c=>({'ם':'מ','ן':'נ','ך':'כ','ף':'פ','ץ':'צ'}[c]||c);function layout(text){const used=new Set(),parts=[];let space=false;for(const c of text){if(/\s/.test(c)){space=true;continue}if(/[א-ת]/.test(c)){const at=[...word].findIndex((v,j)=>!used.has(j)&&norm(v)===norm(c));if(at<0)continue;used.add(at);parts.push({i:at,c,before:space,punct:''});space=false}else if(parts.length&&/[\u0591-\u05BD\u05BF-\u05C2\u05C4-\u05C5\u05C7]/.test(c)){parts[parts.length-1].c+=c}else if(parts.length&&/[׳',]/.test(c))parts[parts.length-1].punct+=c}const gap=parseFloat(getComputedStyle(title).gap)||0,em=parseFloat(getComputedStyle(title).fontSize),spaceWidth=em*.22,widths=parts.map(p=>nodes[p.i].offsetWidth+(p.punct?em*.15:0)),total=widths.reduce((a,b)=>a+b,0)+gap*Math.max(0,parts.length-1)+parts.filter(p=>p.before).length*spaceWidth;let cursor=total/2;const result={};parts.forEach((p,j)=>{if(p.before)cursor-=spaceWidth;result[p.i]={x:cursor-nodes[p.i].offsetWidth/2,c:p.c,punct:p.punct};cursor-=widths[j]+gap});return result}const base=layout(word),a=layout(from),b=layout(to),blend=(1-Math.cos(Math.PI*Math.max(0,Math.min(1,(u-.15)/.45))))/2;const unit=Math.min(38,innerWidth*.045),n=nodes.length;
...
(()=>{const stamp=document.createElement("p");stamp.className="small";stamp.textContent="גרסה 28 · ניקוד חי · ללא שעת חזרה קבועה";document.getElementById("tools").append(stamp)})();

/* Quiet line drawings accompany real listening, never generated sound. */
(()=>{const ns='http://www.w3.org/2000/svg',panel=document.createElement('div');panel.id='listeningSketch';panel.className='listening-sketch';panel.setAttribute('aria-hidden','true');panel.hidden=true;document.getElementById('instruction').after(panel);const svg=document.createElementNS(ns,'svg');svg.setAttribute('viewBox','0 0 320 130');panel.append(svg);
const art={
bird:['M65 78 Q90 45 118 74 Q142 35 170 64 Q193 44 216 60','M111 89 Q135 71 156 91'],
car:['M67 79 L79 58 L123 58 L146 79 L233 79 Q247 81 247 96 L56 96 L56 86 Z','M88 96 A12 12 0 1 0 112 96 A12 12 0 1 0 88 96','M201 96 A12 12 0 1 0 225 96 A12 12 0 1 0 201 96','M45 68 L25 68 M40 80 L16 80'],
window:['M95 17 L225 17 L225 115 L95 115 Z M160 17 L160 115 M95 62 L225 62','M124 42 Q143 26 156 43 Q171 28 189 42'],
ripples:['M145 59 Q160 44 175 59 M127 76 Q160 40 193 76 M107 95 Q160 32 213 95','M158 91 Q161 85 164 91'],
breath:['M58 71 C100 71 105 30 149 30 S204 99 262 68','M65 90 C112 90 119 49 153 49 S211 113 251 89'],
heart:['M160 100 C145 87 91 53 119 30 C140 11 160 37 160 37 C160 37 179 11 201 30 C231 54 178 88 160 100'],
profile:['M114 111 L115 86 Q86 74 103 57 L114 43 Q111 20 140 19 Q174 16 182 48 Q190 78 170 88 L170 113','M139 58 Q148 52 156 57 M124 77 Q135 82 142 76'],
pair:['M62 109 Q64 73 98 73 Q127 73 130 109 M83 61 C57 33 99 14 112 40 C123 64 89 75 83 61','M192 109 Q194 73 228 73 Q257 73 260 109 M213 61 C187 33 229 14 242 40 C253 64 219 75 213 61','M142 65 Q160 57 178 65'],
circle:['M160 34 C219 34 259 64 226 91 C192 119 119 114 88 89 C57 62 102 33 160 34 Z','M111 65 Q135 48 159 65 Q187 84 211 65'],
thread:['M48 83 C69 27 127 29 133 65 C140 110 77 108 83 71 C90 33 217 27 236 69 C257 116 187 114 190 68 C193 40 245 30 272 73']};
let key='',last=-1,held=false,hold=0;const button=document.createElement('button');button.id='sketchToggle';button.className='sketch-toggle';button.textContent='להניח לתנועה לנוח';panel.after(button);button.hidden=true;button.onclick=()=>{held=!held;hold=Math.max(0,state.total-state.left);button.textContent=held?'להמשיך את התנועה':'להניח לתנועה לנוח';button.setAttribute('aria-pressed',String(held))};const previous=window.updateQuietMotion;window.updateQuietMotion=()=>{previous?.();const p=current(),active=p.visual==='listen'&&!state.ground;panel.hidden=!active;button.hidden=!active;if(!active)return;const id=state.i+':'+state.p;if(id!==key){key=id;last=-1;held=false;button.textContent='להניח לתנועה לנוח';button.setAttribute('aria-pressed','false')}const reduced=matchMedia('(prefers-reduced-motion:reduce)').matches;const elapsed=held?hold:Math.max(0,state.total-state.left),cycle=Math.floor(elapsed/30),u=elapsed%30/30,sets=[['ripples','bird','car','window','bird','ripples','window'],['breath','profile','heart','breath','profile','ripples','breath'],['pair','thread','circle','pair','circle','thread','pair']],name=sets[p.ear][cycle%7];if(last!==cycle){last=cycle;svg.replaceChildren();art[name].forEach(d=>{const path=document.createElementNS(ns,'path');path.setAttribute('d',d);svg.append(path)});panel.dataset.drawing=name}panel.style.opacity=reduced?'.6':String(.08+.58*Math.pow(Math.sin(Math.PI*u),.65));svg.style.transform=reduced?'none':`translate(${Math.sin(u*Math.PI*2)*(p.ear===0?17:7)}px,${Math.cos(u*Math.PI*2)*5}px) scale(${.96+Math.sin(u*Math.PI)*.04})`;};window.updateQuietMotion()})();
